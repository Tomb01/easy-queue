export class SmartQueue<T> {
    ...
    enqueue(element: T, cb?: (element: T) => void): this {
      if (this._listener !== null && !this._paused) {
        ...
        if(!this.isEmpty()) this._listener(element, () => {
    ...
    private _next(): void {
      this.dequeue()
      if (!this.isEmpty() && typeof this._listener !== 'undefined' && !this._paused) this._listener(this.front() as T, this._next.bind(this))
    }
